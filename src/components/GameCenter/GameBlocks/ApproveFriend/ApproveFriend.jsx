...
import { getGameFriendRequest, postGameFriendRequestAccept, deleteGameFriendRequestCancel } from "../../../../actions/gameCenterActions";

class ApproveFriend extends Component {
    state = {
        notFound: false,
        anchorEl: null,
        open: false,
    ...
    getData = () => {
        const { match: {params}, getGameFriendRequest } = this.props;
        getGameFriendRequest(params.id).then(res => {
    ...
    acceptOpponent = () => {
        const { postGameFriendRequestAccept, match: {params}, history } = this.props;
        let anchorEl = document.getElementById('approve_friend_popper');
        postGameFriendRequestAccept(params.id).then(res => {
            if(res.payload && res.payload.status && res.payload.status === 201) {
                history.push('/admin/game-center/lobby');
            } else if(res.error && res.error.response.status === 400) {
                console.log(res.error.response.data.detail);
                if(
                    res.error.response.data.detail &&
                    res.error.response.data.detail[0] &&
                    res.error.response.data.detail[0] === 'Your balance is low'
                ) {
                    this.setState({open: true, anchorEl, message: res.error.response.data.detail[0]});
                }
            }
            else {
                this.setState({notFound: true});
    ...
    cancelApprove = () => {
        const { deleteGameFriendRequestCancel, match: {params}, history } = this.props;
        deleteGameFriendRequestCancel(params.id).then(res => {
    ...
    render(){
        const { game_center: {friend_request} } = this.props;
        const { notFound, open, anchorEl, message } = this.state;

        if(notFound) return <NotificationExpired/>;
        else if(!friend_request.bet) return <Preloader/>;
        const { opponent, game, ready, players_ready, platform, bet } = friend_request;
        return (
            <TransitionedBlock>
                <div className="block_center">
                    <div className="approve_opponent_info block_approve_wrapper">
                        ...
                        <div className="bottom_opponent_info">
                            <div id="approve_friend_popper" className="opponent_approve_block">
                                <DefaultButton
                                    type="green"
...
const mapDispatchToProps = {
    getGameFriendRequest,
    postGameFriendRequestAccept,
    deleteGameFriendRequestCancel
};
